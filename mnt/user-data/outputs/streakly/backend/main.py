...
async def create_payment(body: CreatePaymentRequest, user=Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            f"streakly-{payment_id}", payment_id
        )
    ...
    return {
        ...
        "kaspiDeepLink": f"kaspi://payment?merchantId={KASPI_MERCHANT_ID}&amount={plan_info['amount']}&orderId=streakly-{payment_id}",
...
async def kaspi_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    order_id  = body.get("orderId", "")    # "streakly-42"
...
@app.get("/")
async def root():
    return {"app": "Streakly API", "version": "2.0.0", "docs": "/docs"}
