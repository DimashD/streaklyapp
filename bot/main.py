"""
Streaklyapp Bot v2 — aiogram 3.x
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if user.get("consent_given") and habits:
        ...
        await msg.answer(
            ...
            "Я — *Streaklyapp* 🔥 Твой трекер привычек.\n\n"
...
async def finish_onboarding(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"🚀 *Добро пожаловать в Streaklyapp!*\n\n"
        f"Твои привычки:\n{list_str}\n\n"
...
async def cmd_refer(msg: Message):
    ...
    await msg.answer(
        ...
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="📤 Поделиться ссылкой", switch_inline_query=f"Присоединяйся к Streaklyapp — трекеру привычек! {ref_link}")
        ]])
    )
...
async def export_data(event, **kw):
    ...
    await msg.answer_document(
        BufferedInputFile(json_bytes, filename=f"streaklyapp_{date.today()}.json"),
...
async def delete_confirmed(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "✅ Все данные удалены.\n\n"
        "Спасибо что пользовался Streaklyapp!\n"
...
async def main():
    ...
    logger.info("🚀 Streaklyapp Bot v2 starting...")
